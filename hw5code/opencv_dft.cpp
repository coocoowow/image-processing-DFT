...
{  
    ...
    int M = getOptimalDFTSize( grayimage.rows );                               // 获得最佳DFT尺寸，为2的次方  
    int N = getOptimalDFTSize( grayimage.cols );                                 //同上  
    Mat padded;  
    copyMakeBorder(grayimage, padded, 0, M - grayimage.rows, 0, N - grayimage.cols, BORDER_CONSTANT, Scalar::all(0));   // opencv中的边界扩展函数，提供多种方式扩展  
